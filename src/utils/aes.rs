...
pub fn gcm_encrypt<M: ReducePoly>(nonce: &[u8; 12], key: &[u8; 16], mut plaintext: Vec<u8>, mut ad: Vec<u8>) -> Result<AesGcmResult> {
    ...
    let plain_len = (plaintext.len() as u64) * 8;
    encrypter.pad(false);
    ...
    pad_vec(&mut ad);
    for i in 0..(ad.len() / 16) {
        let block = get_16_bytes(&ad[i..(i+16)])?;
        auth_tag = GF2m::<M>::new(auth_tag.value ^ u128::from_be_bytes(block));
        auth_tag = &auth_tag * &auth_key_gf;
    }

    let cipher_offset = plaintext.len() % 16;
    let ciphertext_len = if cipher_offset == 0 { plaintext.len() } else  { plaintext.len() + (16 - cipher_offset) };
    pad_vec(&mut plaintext);
    let mut ciphertext: Vec<u8> = Vec::new();
    for i in 0..(ciphertext_len / 16) {
    ...
    let l = (ad_len as u128) << 64 | plain_len as u128;
...
fn pad_vec(input: &mut Vec<u8>) {
    let offset = input.len() % 16;
    if offset == 0 {
        return;
    }
    let pad_len = 16 - offset;
    input.extend(iter::repeat(0u8).take(pad_len));
}
